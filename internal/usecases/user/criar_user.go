...
type CriarUserRequest struct {
	UserName string `json:"username"`
	Email    string `json:"email"`
	Password string `json:"password"`
}

func (req CriarUserRequest) Valid(ctx context.Context) validador.Avaliador {
	var a validador.Avaliador

	a.VerificarCampo(validador.NaoPoderEstaEmBranco(req.UserName), "username", "UserName não pode esta em branco")
	...
	a.VerificarCampo(validador.MinimoCaracteres(req.UserName, 3) && validador.MaximoCaracteres(req.Password, 50), "passwoerd", "Password deve ter um comprimento entre 3 e 50 ")
